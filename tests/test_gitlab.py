...
class GitLabDiffRangeTest(unittest.TestCase):
    def test_TARGET_BRANCH_SHA_기반_동적_머지베이스_계산(self):
        import os
        from unittest.mock import patch

        from forge import GitLabForge

        forge = GitLabForge("test-token")
        with patch.dict(
            os.environ,
            {
                "CI_MERGE_REQUEST_TARGET_BRANCH_SHA": "target_sha_123",
                "CI_MERGE_REQUEST_DIFF_BASE_SHA": "old_base_sha_456",
            },
        ), patch("subprocess.run") as mock_run:
            mock_run.return_value.returncode = 0
            mock_run.return_value.stdout = "dynamic_merge_base_789\n"

            result = forge.diff_range()
            self.assertEqual(result, "dynamic_merge_base_789...HEAD")
            mock_run.assert_called_once_with(
                ["git", "merge-base", "target_sha_123", "HEAD"],
                capture_output=True,
                text=True,
            )

    def test_TARGET_BRANCH_SHA_실패_시_DIFF_BASE_SHA_폴백(self):
        import os
        from unittest.mock import patch
        ...
        with patch.dict(
            ...
            result = forge.diff_range()
            # merge-base 실패 시 기존 DIFF_BASE_SHA 로 폴백
            self.assertEqual(result, "old_base_sha_456...HEAD")

    def test_DIFF_BASE_SHA만_있을_때_정상_반환(self):
        import os
        from unittest.mock import patch
        ...
        with patch.dict(
            os.environ,
            {"CI_MERGE_REQUEST_DIFF_BASE_SHA": "base_sha_456"},
            clear=True,
        ):
            result = forge.diff_range()
            self.assertEqual(result, "base_sha_456...HEAD")
