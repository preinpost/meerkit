...
게시 전에 `git diff --unified=3` 을 분석하여 diff 에 그려지는 라인에 속한 지적만 선별하여 전송한다.
변경된 라인뿐 아니라 hunk 안의 컨텍스트 라인도 대상에 포함된다. 변경 주변의 기존 코드를 짚는 지적이
흔한데, 그 줄들도 diff 에 함께 표시되므로 인라인으로 붙일 수 있다. GitLab 은 컨텍스트 라인에
`position[old_line]` 을 함께 주지 않으면 400 을 내므로, 라인 맵에서 변경 전 번호를 같이 계산하여 넘긴다.
컨텍스트 폭은 두 플랫폼이 기본으로 그려주는 3줄이며, `MEERKIT_DIFF_CONTEXT` 로 조정할 수 있다.
그 밖의 라인을 가리킨 지적은 요약의 `라인에 달 수 없는 리뷰` 절로 내려간다.
...
변경 규모가 지나치게 큰 MR/PR 은 LLM 이 문맥을 파악하기 어렵고, 수십만 개의 토큰을 소비하여
계정의 5시간 사용량 쿼터를 빠르게 고갈시킨다.

`meerkit` 은 모델을 호출하기 전에 `git diff --numstat` 으로 변경 규모를 사전 분석한다.
이때 `pnpm-lock.yaml`, `package-lock.json`, `*.min.js` 등 기계적으로 불어나는 락 파일과
자동 생성 파일은 계산에서 제외하여 억울하게 건너뛰어지는 현상을 방지한다.

- **기본 임계치**: 순수 변경 라인 **1,200줄** 초과 또는 변경 파일 **40개** 초과
- **동작 방식**: 기준을 초과하면 모델(`pi`)과 Meridian 프록시 호출을 **완전히 건너뛰어(토큰 소모 0)**,
  MR/PR 상단에 분할 권고 안내 코멘트(P0)만 즉시 게시하고 파이프라인을 성공으로 종료한다.
- **설정 조정**: 환경변수로 상한을 변경(`MEERKIT_MAX_LINES=2000`, `MEERKIT_MAX_FILES=60`)하거나,
  `0` 으로 지정하여 크기 제한을 해제할 수 있다.
